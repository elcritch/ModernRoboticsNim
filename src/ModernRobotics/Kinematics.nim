...
func fKinBody*[I: static[int], V](Mat: HomogenousMatrix[V], Blist: StaticMatrix[
  ...
  for col in Blist.columns:
    tmp = tmp * exp6((toSe3((col * thetalist[i])))).asDynamic
    inc i
...
func fKinSpace*[I: static[int], V](
  ...
  for col in Slist.asDynamic.reverseColumns:
    tmp = exp6((toSe3((col * thetalist[i]).asStatic(6)))).asDynamic * tmp
    dec i

  result = tmp.asStatic(4, 4)

func jacobianBody*[I: static[int], V](
    blist: StaticMatrix[6, I, V],
    thetalist: StaticVector[I, V]
): StaticMatrix[6, I, V] =
...
func jacobianSpace*[I: static[int], V](
      slist: StaticMatrix[6, I, V],
      thetalist: StaticVector[I, V]
): StaticMatrix[6, I, V] =

...
proc numericalformatter*[T](mat: T; precision = 2; trim = false;
        center = 10): string =
  mat.mapit( (it.truncate.formateng(precision, trim = trim)).center(
          center)).join(" ")

proc pretty*[N, M, V](mat: StaticMatrix[N, M, V]; precision = 2; trim = false;
        center = 10): string =
...
proc pretty*[N, V](mat: StaticVector[N, V]; precision = 2; trim = false;
        center = 10): string =
...
proc pretty*[V](mat: (RotationMatrix[V], PositionVec[V]); precision = 2;
        trim = false; center = 10): string =
...
template norm*[V](v: TriVector[V]): auto =
  v.l_2
...
proc outer*[N, M, V](u: StaticVector[N, V]; v: StaticVector[M,
...
func pinv*[V](G: Matrix[V]): Matrix[V] =
  ...
  for k in 0 .. n - 1:
    ...
    if r == 0:
      ...
      L[k..n - 1, r..r] = A[k .. n-1, k..k] - L[k .. n - 1, 0 .. (r-1)] *
              L[k..k, 0 .. (r-1)].T

...
func ad*[A](V: HexVector[A]): HexMatrix[A] =
  ...
  ##     [-5,  4,  0, -2,  1,  0]])
  ## 
  let omgmat = toSo3(vector([V[0], V[1], V[2]])).asDynamic
  return vstack(hstack(omgmat, zeros(3, 3).asDynamic),
                hstack(toSo3(vector([V[3], V[4], V[5]])).asDynamic,
                omgmat)
         ).asStatic(6, 6)
...
func inverseDynamics*[N: static[int], V](
  ...
  AdTi[N] = Mlist[n].transInv().adjoint()
  ...
  for i in 0..(N - 1):
    ...
    Vdi[All, ++i .. ++i] = (AdTi[i].asDynamic * Vdi[All, i..i]) +
        ...
        ((ad(Vi[All, ++i .. ++i].asVector.asStatic(6)).asDynamic * (Ai[All,
            i..i] * dthetalist[i])))
  ...
  for i in countdown(n - 1, 0):
    ...
    (ad(Vi[All, ++i .. ++i].asVector.asStatic(6)).T.asDynamic * (Glist[
        i].asDynamic * Vi[All, ++i .. ++i])))
...
func gravityForces*[N, V](
  ...
  ## vercome gravity at its configuration
  ## 
  ## param thetalist: A list of joint variables
...
func endEffectorForces*[N, V](
  ...
  ## reate the end-effector force Ftip
  ## 
  ## param thetalist: A list of joint variables
  ## param Ftip: Spatial force applied by the end-effector expressed in frame
  ##  {n+1}
...
func forwardDynamics*[N, V](
  ...
  return inv(massMatrix(thetalist, Mlist, Glist, Slist)) * (taulist -
                 velQuadraticForces(thetalist, dthetalist, Mlist, Glist,
                     Slist) -
...
func inverseDynamicsTrajectory[N, n, V](
    thetamat: StaticMatrix[N, n, V],
